...
export default function M600Page() {
  const { lang, setLang } = useLanguage();
  const [activeTab, setActiveTab] = useState('specs');

  const content = siteContent[lang];
  const isRtl = lang === 'fa';
  const product = content.products.items.find((p) => p.id === 'm600');
  ...
  const handleDownloadCatalog = () => {
    ...
    link.href = '/catalogs/m600-linux.pdf';
    link.download = 'M600-Linux-Catalog.pdf';
    link.click();
  };
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-b-2xl shadow-sm p-6 sm:p-8 mb-8">
            {activeTab === 'specs' && (
              <div className="space-y-6">
                <h3 className="text-2xl font-bold text-slate-900 mb-4">
                  {isRtl ? 'مشخصات فنی کامل M600' : 'M600 Complete Technical Specifications'}
                </h3>
            ...
            {activeTab === 'features' && (
              <div className="space-y-6">
                <h3 className="text-2xl font-bold text-slate-900 mb-4">
                  {isRtl ? 'قابلیت‌ها و ویژگی‌های M600' : 'M600 Features & Capabilities'}
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {activeTab === 'security' && (
              <div className="space-y-6">
                <h3 className="text-2xl font-bold text-slate-900 mb-4">
                  {isRtl ? 'امنیت و گواهینامه‌های M600' : 'M600 Security & Certifications'}
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="p-5 bg-emerald-50 rounded-xl border border-emerald-200">
                    <ShieldCheck className="w-8 h-8 text-emerald-600 mb-3" />
                    <div className="text-sm font-bold text-slate-900">CE & RoHS Certified</div>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="bg-gradient-to-r from-primary to-primary-dark rounded-3xl p-8 text-center shadow-xl">
            <h3 className="text-2xl font-bold text-white mb-3">
              {isRtl ? 'آماده سفارش M600 هستید؟' : 'Ready to Order M600?'}
            </h3>
